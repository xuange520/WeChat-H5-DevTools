...
**`WeChat-H5-DevTools`** 是一套开箱即用的工业级全能解决方案，集 **微信内免按键浮动调试器注入**、**微信内核原生 CDP 远程调试服务 (直连微信原生窗口免弹窗)**、**脱机高保真 JSSDK 模拟沙箱**、**本地代码实时热重载 (Local Overrides)**、**自动化全量 AST 语法树深度解混淆与 Webpack 模块解包** 以及 **全域 API 路由与国密算法静态审计** 于一体，全面赋能微信 Web 生态开发与安全审计！
...
* **需求 B：想直接调试微信内置文章窗口，按 F12 打断点或自动化操控？**
  * 在命令行输入：
    ```bash
    wx-h5 open
    ```
...
针对微信 4.x 内置浏览器对公众号推文与 H5 页面的安全隔离机制，本节提供权威的标准作业程序（SOP）、白板病因剖析与 4 大最快调试操作顺序。

### 一、 核心原理解析：为什么初次进入 DevTools 显示 about:blank 且 Network 为空？

很多开发者在初次启动 CDP 调试服务并点击 inspect 时，常常遇到 DevTools 窗口显示 `about:blank` 白板且网络面板空空如也的困惑。对照底层通信机制，根因如下：

1. **CDP 默认初始占位靶标机制**：
   - 执行 `wx-h5 open` 启动调试中继服务后，若微信尚未与 CDP 管道建立活跃的页面交互，`chrome://inspect` 中展示并激活的是底层调试服务初始化的空白白板上下文（Target URL 默认显示为 `about:blank`），并不是你正在阅读的文章页面；
2. **Chromium 渲染容器与进程隔离**：
   - 微信体系中，**微信小程序** 运行在独立的 `flue.dll` (RadiumWMPF 引擎) 容器内，而**微信公众号推文 / H5 网页** 是由微信独立的 Chromium 渲染窗口呈现，两者的进程树与 DevTools 靶标彼此独立；
3. **DevTools Network 面板只录制“连接建立后”的流量**：
   - DevTools 的「网络」(Network) 标签页当前停留在录制状态，界面提示 *“目前正在录制网络活动，可重新加载页面或按 Ctrl+R”*。由于 DevTools 是在文章打开**之后**才连接挂载的，文章初次加载时的网络请求已经完成，因此列表暂时为空，只需刷新页面即可重新捕获全部网络流。

---

### 二、 微信公众号推文 / H5 页面 4 大最快调试操作顺序
...
| **方式 2 (零延迟原生)** | 微信文章窗口<br>直接空白处右键「检查」 | 审查微信文章原生 DOM 结构、CSS 样式与控制台调试 | 原地弹出微信官方内置满血 DevTools，100% 真实原生上下文 | 3 秒 |
...
#### 方式 2 详解：在微信文章窗口直接右键「检查」唤起原生 DevTools
1. **步骤 1**：在电脑微信中正常打开公众号推文或网页窗口；
2. **步骤 2**：将鼠标移动至文章页面任意空白区域（文字段落或图片周围空白处）；
3. **步骤 3**：单击鼠标**右键**，在弹出的右键菜单中点击 **「检查」 (Inspect)** 或 **「审查元素」**；
4. **步骤 4**：微信将直接原地弹出独立的 Chromium 原生开发者工具窗口，零转发、100% 满血原生环境，直接开启断点与网络分析！
...
* **微信公众号内核浏览器 CDP 调试服务**：通过 `wx-h5 open` 启动微信内核 CDP 调试服务，暴露 `62000` 端口 CDP，直连微信内置原生窗口（免弹外部浏览器），在 Edge/Chrome 输入 `edge://inspect` 或 `devtools://...` 直接调试微信内置文章窗口，或通过 `miniapp-cdp-mcp` 自动化操控；
...
* **脱机高保真沙箱与 JSSDK 模拟 (--sandbox)**：内置全平台微信 User-Agent 矩阵与 30+ 常见 `WeixinJSBridge` 原生 API Mock（支持支付、扫码、定位、分享拦截），外部 Chrome/Edge 满血开 F12 不报错；
...
| 功能模块 | 界面核心控件与按钮 | 触发动作与后台逻辑 | 适用场景与输出实据 |
| :--- | :--- | :--- | :--- |
| **1. 控制总览 (Dashboard)** | **[刷新内核状态]** | 动态枚举系统当前运行的微信主进程与渲染器，刷新 PID 与内核版本 | 微信重启或多开后重新建立拓扑感知 |
| | **[立即热注入 vConsole]** | 针对指定 URL（或全局）下发 Frida 内存挂载补丁，并激活透明代理 | 微信打开任意文章后刷新，右下角直接浮现绿色 vConsole 按钮 |
| | **[切换透明代理]** | 切换本地 8899 端口代理监听开关（开启 / 挂起） | 灵活控制微信网络流量是否走本地代理注入通道 |
| **2. 内存热注入 (Injector)** | **[功能注入矩阵开关]**<br>• 强制激活绿色 vConsole 按钮<br>• 开启 Eruda 多功能手机调试器<br>• 绕过 JS-SDK 域名白名单签名校验<br>• 解锁右键审查元素上下文菜单 | 配置精细化注入策略开关 | 根据逆向需求自由组合调试器与免签能力 |
| | **[应用注入策略]** | 将上述勾选的策略热下发至目标微信渲染进程 | 策略毫秒级生效，无需重启微信客户端 |
| **3. 沙箱重定向 (Overrides)** | **[添加重定向规则]** | 配置线上 URL 正则（如 `*://.../jweixin.js`）与本地磁盘文件的映射 | 【开发神器】本地修改 JS/CSS 后刷新页面秒级生效，免打包上线 |
| | **[重定向规则管理表]** | 实时查看各规则的命中计数、启用状态与文件路径 | 监控本地文件替换命中情况 |
| **4. 代码解混淆与安全审计** | **[启动 AST 批量反混淆]** | 调用 Babel AST 引擎，执行常量折叠、标识符重命名与 Webpack 单体包拆解 | 将混淆压缩的单文件代码 1:1 还原为模块化清晰工程结构 |
| | **[重新扫描 (敏感凭据审计)]** | 基于启发式正则引擎深度扫描代码中的 AppSecret、Token、AES/RSA 密钥与 API 路由 | 自动化发现代码中的硬编码漏洞与未授权接口，输出表格审计明细 |
| **5. 物理日志管理 (Live Logs)** | **[复制全部日志]** | 将终端与内核当前所有运行日志一键完整提取至系统剪贴板 | 快速向团队或工单汇报完整的运行日志 |
| | **[打开本地日志]** | 调用 Windows 默认编辑器（记事本/VS Code）直接打开本地磁盘日志文件 | 查阅大体积完整历史物理日志 |
| | **[定位日志目录]** | 在 Windows 资源管理器中高亮选中 `records/logs/console_stream.log` | 快速归档、备份或拷贝日志存盘文件 |
| | **[清空屏幕] / [清空磁盘日志]** | 清空前端终端窗口显示，或重置项目本地物理存盘日志文件 | 开始新一轮调试前清理历史噪音 |

---

### 四、 物理日志持久化管理与安全保障

工作台彻底摒弃“内存临时变量打印”的脆弱设计，采用**本地物理双向存盘中继架构**：
...
> **[INFO] 入群提示**：微信扫码即可直接加入官方交流群。若遇二维码过期，请直接添加作者微信 **`Sleep_Plan`**（备注：**DevTools 入群**），由作者拉入官方群聊。
