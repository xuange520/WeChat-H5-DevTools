...
| **4. 本地替换后微信仍显示线上旧代码** | 微信 Chromium 磁盘强缓存与 304 协商 | wechat_h5_devtools/sandbox/browser_launcher.py | 1. 工具默认已重写响应头注入 
o-cache, no-store；<br>2. 在 vConsole 的 Storage 面板点击 **Clear Cache**；<br>3. 在访问 URL 后添加随机参数 ?_t=时间戳 击穿缓存。 |
...
Commit 规范示例：
- eat(injector): add support for RadiumWMPF kernel 26010
- ix(proxy): resolve port conflict auto-fallback mechanism
