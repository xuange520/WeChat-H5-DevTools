...
@cli.command(name="open", help="在独立高保真沙箱浏览器中打开公众号链接 (带满血 F12 + JSSDK Mock)")
@click.argument("url")
@click.option("--browser", "-b", default="edge", type=click.Choice(["edge", "chrome"]), help="使用的浏览器内核")
@click.option("--ua", "-u", default="ios", type=click.Choice(["ios", "android", "windows", "mac"]), help="模拟终端平台")
def open_cmd(url, browser, ua):
    print_banner()
